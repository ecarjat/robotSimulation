...
void PrintBodyCom(const mjModel* m, const mjData* d, const char* body_name) {
  ...
  if (id < 0) {
    std::printf("COM: body '%s' not found\n", body_name);
    return;
  }
  const mjtNum* p = d->xipos + 3 * id;
  std::printf("COM: %s xipos = [%.6f %.6f %.6f]\n", body_name, p[0], p[1], p[2]);
}

void PrintSitePos(const mjModel* m, const mjData* d, const char* site_name) {
  if (!m || !d || !site_name) {
    return;
  }
  int id = mj_name2id(m, mjOBJ_SITE, site_name);
  if (id < 0) {
    std::printf("SITE: '%s' not found\n", site_name);
    return;
  }
  const mjtNum* p = d->site_xpos + 3 * id;
  std::printf("SITE: %s xpos = [%.6f %.6f %.6f]\n", site_name, p[0], p[1], p[2]);
}

void PrintModelCom(const mjModel* m, const mjData* d) {
  if (!m || !d) {
    return;
  }
  // Body 0 is the world; its subtree COM is the model COM.
  const mjtNum* com = d->subtree_com + 3 * 0;
  mjtNum mass = 0;
  if (m->body_subtreemass) {
    mass = m->body_subtreemass[0];
  }
  std::printf("COM: model subtree_com = [%.6f %.6f %.6f] (m), total mass = %.6f kg\n",
              com[0], com[1], com[2], mass);
  std::printf("COM: model subtree_com = [%.3f %.3f %.3f] (mm)\n",
              com[0] * 1000.0, com[1] * 1000.0, com[2] * 1000.0);
}
//---------------------------------------- plugin handling -----------------------------------------
  ...
  while (!sim.exitrequest.load()) {
    if (sim.droploadrequest.load()) {
      ...
      if (dnew) {
        ...
        PrintBodyCom(m, d, "torso_L");
        PrintSitePos(m, d, "H_L");
        PrintModelCom(m, d);
...

    {
      ...
      if (m) {
        ...
        if (sim.run) {
          ...
          if (stepped) {
            // Print equality constraint errors periodically (from efc_pos rows).
            if (m->neq > 0 && d->nefc > 0) {
              static double last_eq_print = -1.0;
              if (last_eq_print < 0 || (d->time - last_eq_print) >= 0.5) {
                std::vector<double> eq_max(m->neq, 0.0);
                ...
                std::printf("EQ_ERR t=%.3f max=%.6g", d->time, max_err);
                if (m->neq <= 8) {
                  std::printf(" [");
                  for (int i = 0; i < m->neq; ++i) {
                    std::printf("%s%.6g", (i ? ", " : ""), eq_max[i]);
                  }
                  std::printf("]");
                }
                std::printf("\n");
                last_eq_print = d->time;
              }
            }
            sim.AddToHistory();
          }
